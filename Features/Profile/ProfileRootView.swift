...
#if DEBUG
            Logger(category: "ProfileImage").debug("[ProfileImage] display updated url=\(newValue ?? "nil")")
#endif
        }
        .onChange(of: presenter.viewState.editorProfileImageDisplayPath) { _, newValue in
#if DEBUG
            Logger(category: "ProfileImage").debug("[ProfileImage] display updated url=\(newValue ?? "nil")")
#endif
        }
