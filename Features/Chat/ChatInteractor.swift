...
enum ChatUploadPolicy {
    static let maxFileCount = FileUploadEndpointPolicy.chatFiles.maxFileCount
    static let maxFileSizeBytes = FileUploadEndpointPolicy.chatFiles.maxFileSizeBytes
}
...
enum ChatUploadValidator {
    static func validateFileCount(existingCount: Int = 0, incomingCount: Int) throws {
        ...
        } catch FileUploadValidationError.tooManyFiles {
...
struct ChatRemoteDataSource: ChatRemoteDataSourceProtocol {
    ...
    func uploadFiles(roomID: String, files: [ChatUploadFile]) async throws -> ChatFileResponseDTO {
        ...
        Logger(category: "MultipartUpload").debug("[MultipartUpload] request path=/v1/chats/{room_id}/files fieldName=\(UploadFieldName.files) fileCount=\(files.count) totalBytes=\(files.reduce(0) { $0 + $1.data.count })")
        ...
        let response = try await apiClient.execute(endpoint)
        Logger(category: "MultipartUpload").debug("[MultipartUpload] response success path=/v1/chats/{room_id}/files")
        return response
    }
}
